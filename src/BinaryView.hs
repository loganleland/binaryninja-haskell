...
foreign import ccall "BNLoadFilename"
  c_BNLoadFilename :: CString    -- filename
...
loadFilename :: String   -- ^ Filename to load
             -> Bool     -- ^ updateAnalysis flag
...
loadFilename filename updateAnalysisB options =
    withCString filename $ \cFilename ->
      withCString options $ \cOptions ->
         c_BNLoadFilename cFilename (if updateAnalysisB then CBool 1 else CBool 0)
                                           cOptions
